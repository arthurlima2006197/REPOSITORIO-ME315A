---
title: "ME315A LAB02"
author: "Arthur Davi Prado de Lima / RA223062"
format: pdf
editor: visual
---
...
Resposta: Para esse problema, as estatísticas suficientes são o número de vôos atrasados (que satisfazem a condição ARRIVAL_DELAY \> 10), e o número total de vôos. De forma que, para obter a porcentagem, a operação necessária é: número de vôos atrasados, dividido pelo total de vôos.
...
getStats = function(input, pos) {
  input %>% filter(AIRLINE %in% c("AA", "DL", "UA", "US"), !is.na(ARRIVAL_DELAY), 
  !is.na(DAY), !is.na(MONTH), !is.na(AIRLINE)) %>%
...
```{r}
library(readr)
mycols = cols_only(AIRLINE = 'c', ARRIVAL_DELAY = 'd', DAY = "i", MONTH = "i")
dados_voos = read_csv_chunked(file = "flights.csv.zip", 
...
computeStats = function(stats) { 
  stats %>%
  group_by(DAY, MONTH, AIRLINE) %>%
  summarise(Perc = sum(voos_atrasados) / sum(total_voos)) %>%
  mutate(Data = as.Date(paste("2015", MONTH, DAY, sep = "-"))) %>%
  select(Cia = AIRLINE, Data, Perc)
}
tabela_final <- computeStats(dados_voos)
```
...
baseCalendario <- function(stats, cia) {
  dados_filtrados <- stats %>% filter(Cia == cia)
  ggcal(dados_filtrados$Data, dados_filtrados$Perc)
}

cAA <- baseCalendario(tabela_final, "AA")
cDL <- baseCalendario(tabela_final, "DL")
cUA <- baseCalendario(tabela_final, "UA")
cUS <- baseCalendario(tabela_final, "US")
